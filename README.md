...
- **Auto-refresh runs in the browser, not on the server.** The checkbox on `/mine` is a per-user preference stored in `localStorage`, scoped on a prefix of the user's pubkey. Every 5 minutes (and on mount) the page walks each opted-in listing; if its `validUntil` is within the next 24 hours and not yet past, it runs the same refresh path the manual button uses with a fixed 30-day renewal. Concurrent triggers are guarded so the same listing can't be refreshed twice at once. Failures don't disable the preference — the next tick retries. **The schedule only runs while the tab is open**, because the App Key never leaves the browser. Closing the tab pauses auto-refresh until you come back.
